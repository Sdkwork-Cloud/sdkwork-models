...
class OpenAiCompletionsToAnthropicMessagesConverter(Converter):
    """OpenAI Chat Completions API → Anthropic Messages API 转换器"""
    ...
    def source_protocol(self) -> Protocol:
        return Protocol.OPENAI_COMPLETIONS

    def target_protocol(self) -> Protocol:
        return Protocol.ANTHROPIC_MESSAGES

    def capabilities(self) -> list[Capability]:
        return [Capability.STREAM, Capability.TOOLS, Capability.CODE, Capability.REASONING]
    ...
    async def convert_response(self, response: ConversionResponse) -> ConversionResponse:
        ...
        return response.model_copy(
            update={
                "protocol": Protocol.OPENAI_COMPLETIONS,
                "model": model,
            }
        )
