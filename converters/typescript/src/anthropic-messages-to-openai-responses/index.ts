...
export class AnthropicMessagesToOpenAiResponsesConverter implements Converter {
  ...
  name(): string { return 'ANTHROPIC_MESSAGES_TO_OPENAI_RESPONSES'; }
  sourceProtocol(): Protocol { return 'anthropic_messages' as Protocol; }
  targetProtocol(): Protocol { return 'openai_responses' as Protocol; }
  capabilities(): Capability[] { return ['stream', 'tools', 'vision', 'code', 'reasoning'] as Capability[]; }
  ...
  async convertRequest(request: ConversionRequest): Promise<ConversionRequest> {
    ...
    return { ...request, protocol: this.targetProtocol(), model, messages };
  }
