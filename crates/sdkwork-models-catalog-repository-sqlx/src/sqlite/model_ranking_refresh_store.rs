...
#[derive(Debug, Clone)]
pub struct SqliteModelRankingRefreshStore {
    pool: SqlitePool,
}

impl SqliteModelRankingRefreshStore {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}

impl ModelRankingRefreshStore for SqliteModelRankingRefreshStore {
...
async fn load_ranking_aggregates(
            ...
            FROM (
                SELECT
                    ...
                    ROW_NUMBER() OVER (
                        ...
                        ORDER BY r.snapshot_date DESC, r.snapshot_period DESC, r.rank_no ASC
                    ) AS previous_row_no
    ...
    .bind(normalize_iso_timestamp(&command.window_start))
    .bind(normalize_iso_timestamp(&command.window_end))
    .bind(limit)
    .fetch_all(&mut **tx)
    .await
...
async fn deactivate_existing_snapshot(
    ...
    sqlx::query(
        ...
        WHERE tenant_id = ?
          ...
          AND snapshot_date = ?
          AND snapshot_period = ?
          AND COALESCE(rank_scope, 'commercial-default') = ?
        "#,
    )
