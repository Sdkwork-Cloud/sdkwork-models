...
SELECT
    CAST(COALESCE(r.snapshot_date, '') AS TEXT) AS snapshot_date,
    CAST(COALESCE(r.snapshot_period, '') AS TEXT) AS snapshot_period,
...
WHERE r.status = 1
  AND (?4 IS NULL OR lower(COALESCE(r.vendor_code, '')) = ?4)
  AND (?5 IS NULL OR r.modality = ?5)
  AND (?6 IS NULL OR lower(COALESCE(r.model, '') || ' ' || COALESCE(r.vendor_name_snapshot, '') || ' ' || COALESCE(r.vendor_code, '')) LIKE ?6)
ORDER BY r.rank_no ASC, r.id DESC
LIMIT ?7
"#;
...
SELECT
    snapshot_date,
    CAST(DENSE_RANK() OVER (ORDER BY snapshot_date ASC) - 1 AS INTEGER) AS history_index,
    catalog_key,
    model,
    rank_no,
    volume,
...
SELECT
    ...
    COALESCE(r.rank_scope, 'commercial-default') AS rank_scope,
    CAST(COALESCE(r.metadata, '{}') AS TEXT) AS metadata,
    CAST(COALESCE(r.rank_payload, '{}') AS TEXT) AS rank_payload
FROM ai_model_rank_snapshot r
JOIN selected_snapshot s
  ON r.tenant_id = s.tenant_id
 AND r.organization_id = s.organization_id
...
WITH selected_snapshot_scope AS (
    ...
    ORDER BY
        CASE
            ...
            ELSE 0
        END DESC,
        MAX(COALESCE(snapshot_date, '')) DESC,
        MAX(COALESCE(snapshot_period, -1)) DESC
    LIMIT 1
),
selected_fallback_job_scope AS (
    ...
    WHERE status = 1
      ...
      AND lower(COALESCE(json_extract(payload, '$.rankScope'), json_extract(payload, '$.rank_scope'), 'commercial-default')) = ?3
    GROUP BY tenant_id, organization_id
    ORDER BY
        CASE
            WHEN ?1 > 0 AND tenant_id = ?1 AND organization_id = ?2 THEN 3
...
SELECT
    ...
    j.tenant_id,
    j.organization_id,
    CAST(COALESCE(j.started_at, '') AS TEXT) AS started_at,
    CAST(COALESCE(j.ended_at, '') AS TEXT) AS ended_at,
    COALESCE(j.duration_ms, 0) AS duration_ms,
    COALESCE(j.execution_status, 0) AS execution_status,
    COALESCE(j.success_count, 0) AS success_count,
    COALESCE(j.failure_count, 0) AS failure_count,
    NULLIF(failure_reason, '') AS failure_reason,
    CAST(COALESCE(j.payload, '{}') AS TEXT) AS payload
...
WHERE j.status = 1
  ...
  AND j.job_type = ?4
  AND lower(COALESCE(json_extract(j.payload, '$.rankScope'), json_extract(j.payload, '$.rank_scope'), 'commercial-default')) = ?3
ORDER BY COALESCE(j.started_at, '') DESC, j.id DESC
LIMIT ?5
"#;
...
SELECT
    ...
    CAST(COALESCE(ended_at, '') AS TEXT) AS ended_at,
    COALESCE(duration_ms, 0) AS duration_ms,
    COALESCE(execution_status, 0) AS execution_status,
    COALESCE(success_count, 0) AS success_count,
    COALESCE(failure_count, 0) AS failure_count,
    NULLIF(failure_reason, '') AS failure_reason,
    CAST(COALESCE(payload, '{}') AS TEXT) AS payload
FROM ops_job_execution
WHERE status = 1
  AND COALESCE(job_name, '') = 'model_ranking_refresh'
  AND job_type = ?4
...
pub struct SqliteModelRankingsReadStore {
    pool: SqlitePool,
}

impl SqliteModelRankingsReadStore {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}

impl ModelRankingsReadStore for SqliteModelRankingsReadStore {
...
impl ModelRankingRefreshStatusReadStore for SqliteModelRankingsReadStore {
...
impl ModelRankingRefreshJobHistoryReadStore for SqliteModelRankingsReadStore {
...
impl ModelRankingsCacheInvalidator for SqliteModelRankingsReadStore {}
...
fn ranking_item_from_row(index: usize, row: &sqlx::sqlite::SqliteRow) -> ModelRankingItem {
...
fn optional_decimal_cell(row: &sqlx::sqlite::SqliteRow, column: &str) -> Option<f64> {
