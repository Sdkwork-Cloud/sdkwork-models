...
use crate::admin_models_list::{
    optional_non_empty, normalized_search_pattern, LIST_MODELS_BASE_WHERE_SQLITE,
...
#[derive(Debug, Clone)]
pub struct SqliteModelCatalogAdminStore {
    pool: SqlitePool,
...
impl SqliteModelCatalogAdminStore {
...
impl ModelCatalogAdminStore for SqliteModelCatalogAdminStore {
    fn list_vendors<'a>(
        &'a self,
...
async fn apply_sdkwork_models_catalog_refresh(
    ...
    crate::sqlite::model_catalog_import::import_sqlite_model_catalog_tx(tx, catalog)
        .await
...
async fn list_model_mappings(
    ...
    let rows = sqlx::query(&mapping_select_sql(
        ...
        WHERE r.tenant_id = ?
          ...
          AND (
              ? IS NULL
              OR r.source_vendor_code = ?
              OR r.target_vendor_code = ?
              OR EXISTS (
                  SELECT 1 FROM ai_model_mapping_rule_binding b
                  WHERE b.rule_id = r.id
                    AND b.tenant_id = r.tenant_id
          ...
          AND (
              ? IS NULL
              OR r.source_vendor_code LIKE '%' || ? || '%'
              OR r.target_vendor_code LIKE '%' || ? || '%'
              OR EXISTS (
                  ...
                  WHERE i.rule_id = r.id
                    ...
                    AND i.status = 1
                    AND (i.source_model LIKE '%' || ? || '%' OR i.target_model LIKE '%' || ? || '%')
              )
              OR EXISTS (
                  SELECT 1 FROM ai_model_mapping_rule_binding b
                  WHERE b.rule_id = r.id
                    ...
                    AND b.deleted_at IS NULL
                    AND b.status = 1
                    AND (COALESCE(b.binding_code, '') LIKE '%' || ? || '%' OR COALESCE(b.binding_name_snapshot, '') LIKE '%' || ? || '%')
              )
          )
        ORDER BY
          CASE COALESCE((
...
fn vendor_select_sql(predicate: &str) -> String {
    format!(
        ...
        SELECT
            ...
            COALESCE(supported_protocols, '[]') AS supported_protocols,
            COALESCE(client_api_compatibility, '{{}}') AS client_api_compatibility,
            status,
...
fn mapping_match_select_sql(predicate: &str) -> String {
    format!(
        ...
        SELECT
            ...
            COALESCE(i.enabled, 0) AS item_enabled,
